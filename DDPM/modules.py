import logging

import numpy as np
import torch
import torch.nn as nn
import torch.nn.functional as F
from einops import rearrange

log = logging.getLogger(__name__)


class EfficientMaskedConv1d(nn.Module):
    """
    1D Convolutional layer with masking capability.
    """

    def __init__(
        self,
        in_channels,
        out_channels,
        kernel_size,
        mask=None,
        bias=True,
        padding_mode="zeros",
    ):
        super().__init__()

        if mask is None:
            self.layer = nn.Conv1d(
                in_channels,
                out_channels,
                kernel_size,
                bias=bias,
                padding="same",
                padding_mode=padding_mode,
            )
        else:
            self.layer = MaskedConv1d(
                in_channels,
                out_channels,
                kernel_size,
                mask,
                bias=bias,
                padding_mode=padding_mode,
            )

    def forward(self, x):
        return self.layer.forward(x)


class MaskedConv1d(nn.Module):
    """
    1D Convolutional layer with masking capability.
    """

    def __init__(
        self,
        in_channels,
        out_channels,
        kernel_size,
        mask,
        bias=True,
        padding_mode="zeros",
    ):
        super().__init__()
        assert (out_channels, in_channels) == mask.shape

        self.padding_mode = "constant" if padding_mode == "zeros" else padding_mode
        total_padding = kernel_size - 1
        left_pad = total_padding // 2
        self.pad = [left_pad, total_padding - left_pad]  # Calculate padding sizes
        init_k = np.sqrt(1.0 / (in_channels * kernel_size))  # Initialize weights
        self.weight = nn.Parameter(
            data=torch.FloatTensor(out_channels, in_channels, kernel_size).uniform_(
                -init_k, init_k
            ),
            requires_grad=True,
        )
        self.register_buffer("mask", mask)  # Non-trainable mask
        self.bias = (
            nn.Parameter(
                data=torch.FloatTensor(out_channels).uniform_(-init_k, init_k),
                requires_grad=True,
            )
            if bias
            else None
        )  # Initialize bias

    def forward(self, x):
        return F.conv1d(
            F.pad(x, self.pad, mode=self.padding_mode),  # Apply padding
            self.weight * self.mask.unsqueeze(-1),  # Apply mask to weights
            self.bias,
        )


class SLConv(nn.Module):
    """
    Structured Long Convolutional layer.
    Adapted from https://github.com/ctlllll/SGConv

    Args:
        kernel_size: Base kernel size
        num_channels: Number of channels
        num_scales: Number of scales (total length = kernel_size * (2 ** (num_scales - 1)))
        decay_min: Minimum decay factor (advanced)
        decay_max: Maximum decay factor (advanced)
        heads: Number of attention heads
        padding_mode: Padding mode ("zeros" or "circular")
        use_fft_conv: Whether to use FFT-based convolution
        interpolate_mode: Interpolation mode ("nearest" or "linear") (advanced)
    """

    def __init__(
        self,
        kernel_size,
        num_channels,
        num_scales,
        decay_min=2.0,
        decay_max=2.0,
        heads=1,  # Number of attention heads
        padding_mode="zeros",
        use_fft_conv=False,
        interpolate_mode="nearest",
    ):
        super().__init__()
        assert decay_min <= decay_max

        self.h = num_channels
        self.num_scales = num_scales
        self.kernel_length = kernel_size * (2 ** (num_scales - 1))

        self.heads = heads

        self.padding_mode = "constant" if padding_mode == "zeros" else padding_mode
        self.use_fft_conv = use_fft_conv
        self.interpolate_mode = interpolate_mode

        self.D = nn.Parameter(torch.randn(self.heads, self.h))  # Skip connection parameter

        total_padding = self.kernel_length - 1
        left_pad = total_padding // 2
        self.pad = [left_pad, total_padding - left_pad]  # Initialize padding

        # Initialize convolution kernels
        self.kernel_list = nn.ParameterList()
        for _ in range(self.num_scales):
            kernel = nn.Parameter(torch.randn(self.heads, self.h, kernel_size))
            self.kernel_list.append(kernel)

        # Support multiple scales (non-sparse setting)
        self.register_buffer(
            "multiplier",
            torch.linspace(decay_min, decay_max, self.h).view(1, -1, 1),  # Decay factors
        )
        self.register_buffer("kernel_norm", torch.ones(self.heads, self.h, 1))  # Normalization factor
        self.register_buffer(
            "kernel_norm_initialized", torch.tensor(0, dtype=torch.bool)
        )

    def forward(self, x):
        signal_length = x.size(-1)

        kernel_list = []
        for i in range(self.num_scales):
            kernel = F.interpolate(
                self.kernel_list[i],
                scale_factor=2 ** (max(0, i - 1)),
                mode=self.interpolate_mode,
            ) * self.multiplier ** (self.num_scales - i - 1)
            kernel_list.append(kernel)
        k = torch.cat(kernel_list, dim=-1)  # Concatenate all scale kernels

        if not self.kernel_norm_initialized:
            self.kernel_norm = k.norm(dim=-1, keepdim=True).detach()
            self.kernel_norm_initialized = torch.tensor(
                1, dtype=torch.bool, device=k.device
            )
            log.debug(f"Kernel norm: {self.kernel_norm.mean()}")
            log.debug(f"Kernel size: {k.size()}")

        assert k.size(-1) < signal_length
        if self.use_fft_conv:
            k = F.pad(k, (0, signal_length - k.size(-1)))

        k = k / self.kernel_norm  # Normalize kernels

        # Convolution operation
        if self.use_fft_conv:
            if self.padding_mode == "constant":
                factor = 2  # Zero-padding on both ends
            elif self.padding_mode == "circular":
                factor = 1  # Circular signal

            k_f = torch.fft.rfft(k, n=factor * signal_length)  # Kernel frequency domain
            u_f = torch.fft.rfft(x, n=factor * signal_length)  # Input frequency domain
            y_f = torch.einsum("bhl,chl->bchl", u_f, k_f)  # Frequency domain convolution
            slice_start = self.kernel_length // 2
            y = torch.fft.irfft(y_f, n=factor * signal_length)  # Convert back to time domain

            if self.padding_mode == "constant":
                y = y[..., slice_start : slice_start + signal_length]  # (B C H L)
            elif self.padding_mode == "circular":
                y = torch.roll(y, -slice_start, dims=-1)
            y = rearrange(y, "b c h l -> b (h c) l")

        else:
            # PyTorch uses cross-correlation - flip kernel for convolution
            y = F.conv1d(
                F.pad(x, self.pad, mode=self.padding_mode),
                rearrange(k.flip(-1), "c h l -> (h c) 1 l"),
                groups=self.h,
            )  # Standard convolution

        # Add skip connection (D term in state space equation)
        y = y + rearrange(
            torch.einsum("bhl,ch->bchl", x, self.D),
            "b c h l -> b (h c) l",
        )

        return y


### Sparse mask generation functions ###


def get_in_mask(
    signal_channel,
    hidden_channel,
    cond_channel=0,
):
    """
    Generate input mask for specified configuration.

    Args:
        signal_channel: Number of signal channels
        hidden_channel: Number of hidden channels
        cond_channel: Number of conditioning channels
    Returns:
        Input mask tensor
    """
    np_mask = np.concatenate(
        (
            get_restricted(signal_channel, 1, hidden_channel),  # Restricted connection for EEG
            get_full(cond_channel, signal_channel * hidden_channel),  # Full connection for conditioning
        ),
        axis=1,
    )
    return torch.from_numpy(np.float32(np_mask))


def get_mid_mask(signal_channel, hidden_channel, off_diag, num_heads=1):
    """
    Generate hidden layer mask.

    Args:
        signal_channel: Number of signal channels
        hidden_channel: Number of hidden channels
        off_diag: Number of off-diagonal interactions
        num_heads: Number of attention heads
    Returns:
        Hidden layer mask tensor
    """
    np_mask = np.maximum(
        get_restricted(signal_channel, hidden_channel, hidden_channel),
        get_sub_interaction(signal_channel, hidden_channel, off_diag),
    )

    return torch.from_numpy(np.float32(np.repeat(np_mask, num_heads, axis=1)))


def get_out_mask(signal_channel, hidden_channel):
    """
    Generate output mask.

    Args:
        signal_channel: Number of signal channels
        hidden_channel: Number of hidden channels
    Returns:
        Output mask tensor
    """
    np_mask = get_restricted(signal_channel, hidden_channel, 1)
    return torch.from_numpy(np.float32(np_mask))


def get_full(num_in, num_out):
    """Generate full connection mask (all ones)"""
    return np.ones((num_out, num_in))


def get_restricted(num_signal, num_in, num_out):
    """
    Generate block diagonal mask.
    
    Creates a mask with ones only on the block diagonal.
    Equivalent to:
        identity = np.eye(num_signal)
        repeated_rows = np.repeat(identity, num_out, axis=0)
        result = np.repeat(repeated_rows, num_in, axis=1)
    """
    return np.repeat(np.repeat(np.eye(num_signal), num_out, axis=0), num_in, axis=1)


def get_sub_interaction(num_signal, size_hidden, num_sub_interaction):
    """Generate off-diagonal interaction mask"""
    sub_interaction = np.zeros((size_hidden, size_hidden))  # hidden x hidden
    sub_interaction[:num_sub_interaction, :num_sub_interaction] = 1.0  # Define interaction region
    return np.tile(sub_interaction, (num_signal, num_signal))  # Tile to full size